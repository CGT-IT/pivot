...
function _add_section_share($offre) {
  ...
  $offerTitle = _get_urn_value($offre, 'urn:fld:nomofr');
  $output = '<div class="pivot-offer-share">'
    . '<span class="pr-3"><a class="social-icon" href="https://www.facebook.com/sharer.php?u=' . $url_offer_details . '&amp;t=' . $offerTitle . '" target="_blank"><img class="pivot-picto" src="' . get_option('pivot_uri') . 'img/urn:fld:urlfacebook;h=35" alt="Facebook ' . esc_attr__('Share button') . '" title="Facebook ' . esc_attr__('Share button') . '"/></a></span>'
    . '<span><a class="social-icon" href="https://twitter.com/share?text=' . $offerTitle . '&amp;url=' . $url_offer_details . '" target="_blank"><img class="pivot-picto" src="' . get_option('pivot_uri') . 'img/urn:fld:urltwitter;h=35" alt="Twitter ' . esc_attr__('Share button') . '" title="Twitter ' . esc_attr__('Share button') . '"/></a></span>'
    . '</div>';
...
function _add_section_linked_offers($offre) {
  ...
  foreach ($offre->relOffre as $relation) {
    ...
    if (!(in_array($idTypeOffre, array('268', '23', '10')))) {
      // The linked offer type should exist in "pivot offer type" otherwise no template will be used
      if (pivot_get_offer_type($idTypeOffre)) {
        $offerTitle = _get_urn_value($relation->offre, 'urn:fld:nomofr');
        $url = get_bloginfo('wpurl') . '/details/' . $relation->offre->attributes()->codeCgt->__toString() . '&type=' . $idTypeOffre;
        ...
        $output .= '"><blockquote>'
          ...
          . '<span class="offer-title"><small><b>' . $offerTitle . '</b></small></span>'
          . '</div>'
          . '</div>'
          . '</a>'
    ...
    if ($idTypeOffre == 10) {
      ...
      if (pivot_get_offer_type($idTypeOffre)) {
        ...
        $output .= '<div id="' . $relation->offre->attributes()->codeCgt->__toString() . '" class="pivot-rel-offer">'
          ...
          . '<span class="pivot-id-type-offre d-none item">' . $idTypeOffre . '</span>'
          . '<span class="pivot-code-cgt d-none item">' . $relation->offre->attributes()->codeCgt->__toString() . '</span>'
          . '<span class="pivot-latitude d-none item">' . $relation->offre->adresse1->latitude->__toString() . '</span>'
          . '<span class="pivot-longitude d-none item">' . $relation->offre->adresse1->longitude->__toString() . '</span>'
          . '</div>';
  ...
  if ($map == 1) {
    ...
    switch ($nb_col) {
      case 1:
        ...
        break;
      case 2:
        $output = 'col-xl-6 col-lg-6 col-md-6 col-sm-6 col-xs-12 col-12 ';
        break;
      case 3:
        $output = 'col-xl-4 col-lg-4 col-md-6 col-sm-6 col-xs-12 col-12 ';
        break;
      case 5:
        $output = 'w-xl-20 w-lg-20 w-md-20 w-sm-20 w-20 ';
        break;
      case 6:
        $output = 'col-xl-2 col-lg-2 col-md-6 col-sm-6 col-xs-12 col-12 ';
        break;
      default:
        $output = 'col-xl-3 col-lg-4 col-md-6 col-sm-6 col-xs-12 col-12 ';
        break;
    }
  }
